...
class Openstack(plugins.Plugin):
    ...
    def options(self, parser, env):
        ...
        parser.add_option("--openstack-show-elapsed", action="store_true",
                          default=env.get("NOSE_OPENSTACK_SHOW_ELAPSED", 1),
        ...
        parser.add_option("--openstack-color", action="store_true",
                          default=env.get("NOSE_OPENSTACK_COLOR", 1),
...
if __name__ == "__main__":
    os.environ['NOSE_WITH_OPENSTACK'] = '1'
    os.environ['NOSE_OPENSTACK_COLOR'] = '1'
    os.environ['NOSE_OPENSTACK_SHOW_ELAPSED'] = '1'
    os.environ['NOSE_OPENSTACK_STDOUT'] = '1'
    #args = ["", "recipel3", "--re-pattern=test.*|length", "--verbosity=2"]
    args = ["", "recipel3", "--verbosity=2"]

    print "with verbosity..."
    nose.run(argv=args, plugins=[Openstack()])
